...
export async function addInteraction(userInfo, properties) {
    ...
    if (!properties.title || !properties.type) {
      throw { status: 400, message: "Interaction type is required" };
    }
  
    const interactionType = properties.type.toLowerCase();
    ...
    let attendees = fixAttendees(properties.attendees, account);
    ...
    if (properties.timestamp !== undefined) {
      newInteraction.timestamp = properties.timestamp;
    }
  
    if (properties.isSticky !== undefined) {
      newInteraction.isSticky = properties.isSticky;
    }
...
export async function updateInteraction(userInfo, parameters) {
    ...
    let interactionType = properties.type;
    ...
    if (properties.timestamp !== undefined) interaction.timestamp = properties.timestamp;
    if (properties.title !== undefined) interaction.title = properties.title;
    if (properties.description !== undefined) interaction.description = properties.description;
    if (properties.metadata !== undefined) interaction.metadata = properties.metadata;
    if (properties.isSticky !== undefined) interaction.isSticky = properties.isSticky;
    if (properties.attendees) interaction.attendees = attendees;
...
export async function deleteInteraction(userInfo, parameters) {
    const accountId = parameters.account_id;
    const interactionId = parameters.interaction_id;
    const account = await getAccount(userInfo, accountId);
    if (!account) {
