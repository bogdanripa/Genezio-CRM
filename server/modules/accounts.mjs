...
export async function getAccountDetails(userInfo, parameters) {
...
export async function createAccount(userInfo, parameters) {
    const newAccount = {
...
export async function updateAccount(userInfo, parameters) {
    const account = await getAccount(userInfo, parameters.account_id);
    if (!account) {
      const accounts = await getAllAccounts(userInfo);
      const accountNames = accounts.map((a) => `${a.name} (account id: ${a.id})`).join(", ");
